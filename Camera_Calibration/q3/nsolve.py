...
def fdlt(XW,XI):
	...
	A=[]
	for i in range(n):
		t1=[xw[i][0],xw[i][1],xw[i][3],0,0,0,-1*x[i]*xw[i][0],-1*x[i]*xw[i][1],-1*x[i]]

		t2=[0,0,0,xw[i][0],xw[i][1],xw[i][3],-1*y[i]*xw[i][0],-1*y[i]*xw[i][1],-1*y[i]]
		A.append(t1)
		A.append(t2)
	A=np.array(A)
...
for i in range(len(XW)):
	tmp=[XW[i][0],XW[i][1],XW[i][3]]
	nXw.append(tmp)

nXw=np.array(nXw,dtype='float64')
projected=np.dot(out,nXw.T)
...
translation=h3*1/(np.linalg.norm(h1))
print(translation)
